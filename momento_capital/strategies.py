...
def dinorah(
    ...
    filtered_holdings_df, holdings_first_valid_date = process_data(
        ...
        end_date=end_date,
        freq=freq,
        max_window=max(holdings_rsi_window_size, holdings_vol_window_size, manager_sma_window_size),
    )
...
def arjun(
    ...
    filtered_holdings_df, holdings_first_valid_date = process_data(filtered_df=filtered_holdings_df, start_date=start_date, end_date=end_date, freq=freq, max_window=holdings_largest_window)
    filtered_etfs_df, etfs_first_valid_date = process_data(filtered_df=filtered_etfs_df, start_date=start_date, end_date=end_date, freq=freq, max_window=etfs_largest_window)
    ...
    etfs_sma_dfs = [sma_df.loc[sma_df.index >= etfs_first_valid_date] for sma_df in etfs_sma_dfs]
    ...
    for date_idx, date in enumerate(etfs_sma_dates):
        if date_idx % freq == 0:
            ...
            etfs_sma_signal[date] = [
                ...
                for etf in current_active_etfs
                if all(
                    etfs_sma_condition(filtered_etfs_df[etf].loc[date], indicator[etf].loc[date])
    ...
    holdings_long_sma_dates = [date.strftime("%Y-%m-%d") for date in filtered_holdings_df.index]
    ...
    holdings_long_sma_signal = {
        date: {
            holding: {
                "price": float(filtered_holdings_df.loc[date, holding]),
                **{
                    f"sma_{window}": float(holdings_long_sma_dfs[window_idx].loc[date, holding])
    ...
    holdings_short_sma_dates = [date.strftime("%Y-%m-%d") for date in filtered_holdings_df.index]
    ...
    holdings_short_sma_signal = {
        date: {
            holding: {
                "price": float(filtered_holdings_df.loc[date, holding]),
                **{
                    f"sma_{window}": float(holdings_short_sma_dfs[window_idx].loc[date, holding])
                    for window_idx, window in enumerate([holdings_long_sma_window_size])
                },
            }
            for holding in date_signal
